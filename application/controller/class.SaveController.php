...
class SaveController extends Controller
{
  ...
  function executeKernel()
  {
    ...
    foreach($this->_request->getData() as $key => $value)
    {
        ...
        && $value instanceof PersistentObject)
      {
        ...
        foreach ($saveNode->getDataTypes() as $dataType)
        {
          $saveEntry['dataType'] = $dataType;
          foreach ($saveNode->getValueNames($dataType) as $name)
          {
            $saveEntry['name'] = $name;
            $saveEntry['value'] = $saveNode->getValue($name, $dataType);
            ...
            if ($saveEntry['oid'] != '')
            {
              ...
              {
                $properties = $curNode->getValueProperties($saveEntry['name'], $saveEntry['dataType']);

                // remember old value and state ...
                $oldValue = $curNode->getValue($saveEntry['name'], $saveEntry['dataType']);
                $oldState = $curNode->getState();

                // ... and set the new value
                $newValue = $saveEntry['value'];
                if ($oldValue != $newValue) {
                  $curNode->setValue($saveEntry['name'], $newValue, $saveEntry['dataType']);
                }
                // call custom before-save handler
                if ($this->modify($curNode, $saveEntry['name'], $saveEntry['dataType'], $oldValue)) {
                  $needCommit = true;
                }
                // get the modified new value
                $newValue = $curNode->getValue($saveEntry['name'], $saveEntry['dataType']);
                ...
                if (strpos($properties['input_type'], 'file') !== false && strlen($newValue) > 0)
                {
                  ...
                  $uploadDir = $this->getUploadDir($curOID, $saveEntry['name'], $saveEntry['dataType']);
                  ...
                  $fileOk = $this->checkFile($curOID, $saveEntry['name'], $saveEntry['dataType'], $filename);
                }
                ...
                {
                  if ($this->confirmSave($curNode, $saveEntry['name'], $saveEntry['dataType'], $newValue))
                  {
                    // new value is already set, so we just need to commit the change
                    $needCommit = true;
                  }
                  else {
                    $curNode->setValue($saveEntry['name'], $oldValue, $saveEntry['dataType']);
                  }
                }
                else
                {
   ...
   * @param data An assoziative array with keys 'oid', 'name', 'dataType', 'value' where value holds an assoziative
  ...
  function saveUploadFile($data)
  {
    ...
    if ($mediaFile['name'] != '')
    {
      ...
      if ($mediaFile['tmp_name'] != 'none')
      {
        ...
        $uploadDir = $this->getUploadDir($data['oid'], $data['name'], $data['dataType']);

        // check file validity
        if (!$this->checkFile($data['oid'], $data['name'], $data['dataType'], $mediaFile['tmp_name'], $mediaFile['type'])) {
          return false;
        }
        // get the name for the uploaded file
        $uploadFilename = $uploadDir.$this->getUploadFilename($data['oid'], $data['name'], $data['dataType'], $mediaFile['name']);

        // get upload parameters
        $override = $this->shouldOverride($data['oid'], $data['name'], $data['dataType'], $uploadFilename);
   ...
   */
  function checkFile($oid, $valueName, $dataType, $filename, $mimeType=null)
  {
    // check mime type
    if ($mimeType != null)
    {
      $mimeTypes = $this->getMimeTypes($oid, $valueName, $dataType);
    ...
    $imgConstraints = $this->getImageConstraints($oid, $valueName, $dataType);
   ...
   */
  function getMimeTypes($oid, $valueName, $dataType)
  {
    return null;
  }
  /**
   ...
   */
  function getImageConstraints($oid, $valueName, $dataType)
  {
    // get required image dimensions
    $parser = &InifileParser::getInstance();
   ...
   */
  function getUploadFilename($oid, $valueName, $dataType, $filename)
  {
   ...
   */
  function shouldOverride($oid, $valueName, $dataType, $filename)
  {
    return true;
  }
  /**
   ...
   * (section 'media')
   */
  function getUploadDir($oid, $valueName, $dataType)
  {
   ...
   * @param valueType The data type of the value to save.
   ...
   */
  function confirmSave(&$node, $valueName, $valueType, $newValue)
  {
    return true;
  }
  /**
   ...
   */
  function modify(&$node, $valueName, $valueType, $oldValue)
  {
    return false;
  }
  /**
