 ...
 * |
 * | Copyright (C) 2001-2003 Nigel Swinson, nigelswinson@users.sourceforge.net
 * |
...
require_once(dirname(__FILE__).'/dbdebug.php');
...
class CXmlDb {
	...
	*******************************************************************************************/
	...
	// options.
	var $aOptions = array();
	// An array of permissions to execute functions of the class.  The array is an associative
	// array, keyed by function name, with a value which is a number indicating the permissions:
	//		unset -> Defer to default permission
	//		XMLDB_PERMISSION_DISABLE -> Deny operation
	//		XMLDB_PERMISSION_INHERIT -> Defer to default permission
	//		XMLDB_PERMISSION_ENABLE -> Permit operation.
	// The array starts off permitting a very basic level of permission, and the constructor will
	// set all other methods to 1, which will therefore inherit from the $bSecureMode setting.
	var $aPermissions = array(
			'SetOptions'	=> XMLDB_PERMISSION_ENABLE,
			'Open'			=> XMLDB_PERMISSION_ENABLE,
			'Close'			=> XMLDB_PERMISSION_ENABLE
			);
	// This property allows you to set a turn-on or turn-off security policy for the 
	// class.  By default this will be an turn-on policy (TRUE), as this is the most secure.  
	// However if you are just getting started, or don't give two hoots about security, then 
	// blast this to Permit all operations by default (FALSE).  The fine tuning of security
	// policy is done by adjusting the $aPermissions array.
	var $bSecureMode = TRUE;
	...
	*******************************************************************************************/
	...
	function SetOptions($aOptions) {
		if (!$this->_FunctionPermitted('SetOptions')) return FALSE;
	...
	function Open($DbFileName, $bCreateDatabase = FALSE, $bNeedWriteAccess = FALSE) {
		if (!$this->_FunctionPermitted('Open')) return FALSE;
		...
		do {
			...
			if (!file_exists($DbFileName)) {
				...
				if ($bDebugFlag) echo "Creating new database $DbFileName\n";
	...
	function Close($DbFileName = '') {
		if (!$this->_FunctionPermitted('Close')) return FALSE;
...
*/
	 ...
	 *	</XmlDataBase>
	 *
	 * @param $TableTag		(string) The second level tag that contains all the XML that we 
	 *								 consider to represent a table.
	 * @param $RecordTag	(string) The tag that represents the record within the TableTag
	 * @param $aElementData	(array)	 an associative array of name=value pairs which constitue
	 *								 the data of this record
	 * @return				(int)	 The record ID of the new record that was added on success
	 *								 0 on failure
	 */
	function AddRecord($TableTag, $RecordTag, $aElementData) {
		if (!$this->_FunctionPermitted('AddRecord')) return 0;
		...
		do {
			...
			$iResult = $this->_AddRecord($TableTag, $RecordTag, $aElementData);
		...
		$this->Debug->_CloseDebugFunction('AddRecord', $iResult, $bDebugFlag);
		return $iResult;
	}
	 ...
	 * @return				(int)	 The record ID of the new record that was added on success
	 *								 0 on failure, or if it wasn't added as a record already 
	 *								 existed matching the XPathUniqueCriteria
	 */
	function AddUniqueRecord($TableTag, $RecordTag, $aElementData, $XPathSearch) {
		if (!$this->_FunctionPermitted('AddUniqueRecord')) return 0;
		...
		$this->Debug->_CloseDebugFunction('AddUniqueRecord', $iResult, $bDebugFlag);
		return $iResult;
	}
	...
	function ModifyRecord($TableTag, $RecordTag, $RecordId, $aElementData, $bPreserveContent = FALSE) {
		if (!$this->_FunctionPermitted('ModifyRecord')) return FALSE;
	...
	function RemoveRecord($TableTag, $RecordTag, $XPathSearch) {
		if (!$this->_FunctionPermitted('RemoveRecord')) return FALSE;
	...
	function RemoveRecordId($TableTag, $RecordTag, $aRecordIds) {
		if (!$this->_FunctionPermitted('RemoveRecordId')) return FALSE;
	 ...
	 *		)
	 *
	 * @return (array) an array of the table names within the XML file.  FALSE on error.
	 */
	function GetTableNames() {
		if (!$this->_FunctionPermitted('GetTableNames')) return FALSE;
	 ...
	 *							 search in
	 * @return			(array)	 an array of the record names within the $TableTag.  FALSE
	 *							 on error.
	 */
	function GetRecordNames($TableTag) {
		if (!$this->_FunctionPermitted('GetRecordNames')) return FALSE;
	...
	function GetFieldNames($TableTag, $RecordTag) {
		if (!$this->_FunctionPermitted('GetFieldNames')) return FALSE;
	 ...
	 *
	 * This function assists with the creation of forms for records.  You can query the maximum
	 * length of any one field.  If it is multiline you can consider using a textarea instead
	 * of an input control.
	 *
	 * Note that this function may be very slow to call depending on the db implementation.
	 * It should only be called when you need to dynamically discover information about the
	 * schema.  Otherwise you should fix the form sizes.
	 *
	 * @param $TableTag  (string) the second level tag that contains all the XML we are to
	 *							  search in
	 * @param $RecordTag (string) the third level tag that contains all the XML we are to
	 *							  search in
	 * @return			 (array)  An associative array, indexed by field name, with data of
	 *							  length -> Longest data value, in characters
	 *							  multiline -> TRUE if any of the values contain multiple lines
	 *										   FALSE if no data contains a \n character.
	 *							  FALSE on failure.
	 */
	function GetSchema($TableTag, $RecordTag) {
		if (!$this->_FunctionPermitted('GetSchema')) return FALSE;

		// If you are having difficulty using this function.  Then set this to TRUE and
		// you'll get diagnostic info displayed to the output.
		$bDebugFlag = FALSE;

		$this->Debug->_BeginDebugFunction('GetSchema', $bDebugFlag);
		if ($bDebugFlag) {
			echo "TableTag: $TableTag\n";
			echo "RecordTag: $RecordTag\n";
			echo "<hr>";
		...
		// do {} while (false) loop
		do {
			// Do a search for all the records.  We build the schema from the results
			$aSearchResults = $this->Search($TableTag, $RecordTag, "true()");

			// Cycle round, updating the schema as we go.
			foreach($aSearchResults as $RecordID => $aRecord) {
				if ($bDebugFlag) {
					echo "Processing Record $RecordID\n";
					//print_r($aRecord);
				}
				// Cycle round each field in the record.
				foreach ($aRecord as $Field => $Value) {
					// If we haven't come across this field yet, add a new schema entry.
					if (!isset($aResults[$Field])) {
						$aResults[$Field] = array('length' => strlen($Value),
												 'multiline' => (strstr($Value, "\n") !== FALSE));
					} else {
						// If this field has a longer length, use it instead
						if ($aResults[$Field]['length'] < strlen($Value))
							$aResults[$Field]['length'] = strlen($Value);

						// If we haven't found a multiline value yet, then query this one.
						if (!$aResults[$Field]['multiline']) {
							$aResults[$Field]['multiline'] = (strstr($Value, "\n") !== FALSE);
						}
					}
				}
			}
		...
		$this->Debug->_CloseDebugFunction('GetSchema', $aResults, $bDebugFlag);
		return $aResults;
	}
	 ...
	 *								 empty array if no records are found.  FALSE on error.
	 */
	function Search($TableTag, $RecordTag, $XPathSearch) {
		if (!$this->_FunctionPermitted('Search')) return array();
	 ...
	 *
	 * @param $aSearchResults	(array)  The array of data to be searched.
	 ...
	 * @param $ReverseSortOrder	(bool)   Defines the order in which the array is sorted.  The
	 *									 default value of TRUE sorts the array in ascending
	 *									 order.  FALSE will cause the array to be sorted in 
	 *									 reverse order.
	 * @return					(array)	 Associative array of sorted data where a record key
	 *									 will be it's RecordId and the content will be the
	 *									 record data.  FALSE on error
	 */
	function SortSearch($aSearchResults, $SortByKey = 0, $bReverseSortOrder = FALSE) {
		if (!$this->_FunctionPermitted('SortSearch')) return FALSE;
		...
		if ($bDebugFlag) {
			...
			echo "SortOrder: ".($bReverseSortOrder?'Normal':'Reversed')."\n";
			echo "<hr>";
		}
		...
		do {
			...
			if ($aSearchResults) {
				...
				$aSortingArray = array();
				$aRemainder = array();
				// Sorting by record id, although rare, is much easier.
				if ($SortByKey == 'RecordId') {
					foreach($aSearchResults as $RecordId => $ArrayContents) {
						$aSortingArray[$RecordId] = $RecordId;
					}
				} else {
					foreach($aSearchResults as $RecordId => $ArrayContents) {
				...
				foreach($aSortingArray as $SortedKey => $SortedContent)
					...
					$aSortedArray[$SortedKey] = $aSearchResults[$SortedKey];
			}
	 ...
	 *										 is no entries exist.  If $bTestFlag is TRUE, then
	 *										 it will contain the string of the displayed text.
	 */
	function Display($aSearchResults, $FunctionName, $aFunctionParameters, $bTestFlag = FALSE) {
		if (!$this->_FunctionPermitted('Display')) return FALSE;
	...
	function SearchAndDisplay($TableTag, $RecordTag, $XPathSearch, $FunctionName, $aFunctionParameters) {
		if (!$this->_FunctionPermitted('SearchAndDisplay')) return 0;
	...
	*******************************************************************************************/
	 ...
	 * 
	 * @param $MethodName	(string) The name of the method that the user is calling
	 * @return				(bool)   TRUE if the user has permissions to run the query, FALSE otherwise.
	 */
	function _FunctionPermitted($MethodName) {
		// By default (most likely) we are going to look at the secure mode flag to decide 
		// on our action
		$bResult = !$this->bSecureMode;
		// do {} while (false) loop for mini exception handling
		do {
			if (empty($MethodName)) break;
			if (!isset($this->aPermissions[$MethodName])) break;
			switch ($this->aPermissions[$MethodName]) {
			case XMLDB_PERMISSION_ENABLE:
				$bResult = TRUE;
				break;
			case XMLDB_PERMISSION_DISABLE:
				$bResult = FALSE;
				break;
			case XMLDB_PERMISSION_INHERIT:
			default:
				break;
			}
		} while (false);

		if ($this->bErrorReportingFlag && !$bResult)
			trigger_error("The user does not have permission to call the '$MethodName' of this database.");
...
<?
	 ...
	 *
	 * @param $Name		(string) The name of the field we are querying
	 * @return			(bool)	 TRUE if the name was valid.  Else FALSE
	 */
	 ...
	 *
	 * @param $Name		(array)  The proposed element data
	 * @return			(bool)	 TRUE if the names were valid.  Else FALSE
	 */
	function _CheckFieldNames($aElementData) {
		$bDebugFlag = FALSE;

		if (is_array($aElementData)) {
			// Check all the field names of the new record.
			$bBadFieldName = FALSE;
			foreach ($aElementData as $Name => $Value) {
				if (!$this->_IsValidFieldName($Name)) {
					$bBadFieldName = TRUE;
					if ($this->bErrorReportingFlag || $bDebugFlag)
						trigger_error("'$Name' is not a valid Field Name.");
					return FALSE;
				}
			}			
		}
	...
	function _AddRecord($TableTag, $RecordTag, $aElementData) {
		...
		do {
			...
			// Check that all the field names are valid.
			if (!$this->_CheckFieldNames($aElementData)) break;
			...
			if ($aElementData) {
				foreach($aElementData as $Name => $Value) {
					// If the value isn't set, then omit the element completely.  Save
					// on space for what is really an absence of data rather than data.
					if (!isset($Value)) continue;
					// An empty string is the same as "unset".
					if ($Value === '') continue;
		...
		$this->Debug->_CloseDebugFunction('_AddRecord', $iResult, $bDebugFlag);
		return $iResult;
	}
	...
	function _Search($TableTag, $RecordTag, $XPathSearch) {
		...
		do {
			...
			$SearchString = $TablePath."/".$RecordTag."[".$XPathSearch."]";
			$aSearchResults = $this->XmlDb->evaluate($SearchString);

			if ($bDebugFlag) {
				echo "Search for $SearchString produced the following entries:\n";
				print_r($aSearchResults);
				echo "\n";
			}
			...
			foreach ($aSearchResults as $Record) {
				...
				while (strstr(($Element = current($aSearchResultsElements)),$Record)) {
					...
					if ($Value) {
						...
						$Name = $aMatches[3];
						if ($Name) {
							// Remove the HTML entities from the result.
							$Value = $this->XmlDb->decodeEntities($Value);
							if ($bDebugFlag) echo "$Record has $Name data of ".htmlspecialchars($Value)."\n";
