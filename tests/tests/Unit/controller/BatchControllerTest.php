 ...
 */
class BatchControllerTest extends \Codeception\Test\Unit {

  const CONTROLLER = 'wcmf\application\controller\SimpleBatchController';
  ...
  private function process($expectations, $data) {
    ...
    for($i=0, $count=sizeof($expectations); $i<$count; $i++) {
      ...
      if ($i==0) {
        $response = $this->tester->runRequest($expectation[0], self::CONTROLLER, $data, false);
      }
      else {
        $response = $this->tester->runRequestFromThis($expectation[0], self::CONTROLLER, $data, false);
      }
      // test
